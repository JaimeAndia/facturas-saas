      ...
      >
        <div className="flex items-center gap-2.5">
          <div className={`h-2 w-2 rounded-full ${
            !status ? 'bg-gray-300' : status.connected ? 'bg-emerald-500' : 'bg-red-400'
          }`} />
          <h2 className="text-sm font-semibold text-gray-900">Estado XRP Ledger</h2>
          {status?.network && (
            <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
              isTestnet ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700'
            }`}>
      ...
      {abierto && (
        <div className="border-t border-gray-100 px-5 py-4">
          {cargando ? (
            ...
            <p className="text-sm text-red-500">No se pudo obtener el estado de XRPL.</p>
          ) : status ? (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {/* Conexión */}
              <div>
                <p className="text-xs text-gray-400">Conexión</p>
              ...
              {/* Balance */}
              <div>
                <p className="text-xs text-gray-400">Saldo app wallet</p>
              ...
              {/* Precio XRP */}
              <div>
                <p className="text-xs text-gray-400">Precio XRP</p>
                <p className="mt-0.5 text-sm font-semibold text-gray-900">
                  {status.xrp_price_eur > 0 ? `${status.xrp_price_eur.toFixed(4)} €` : '—'}
                </p>
                {status.activations_paused && (
                  <p className="text-xs font-medium text-red-500">Activaciones pausadas</p>
                )}
              </div>

              {/* Address */}
              <div>
                <p className="text-xs text-gray-400">Dirección wallet</p>
                <p className="mt-0.5 break-all font-mono text-xs text-gray-700">{status.address}</p>
                {explorerUrl && (
                  <a
                    href={explorerUrl}
                    target="_blank"
